...
impl DeviceCodeAuth {
    ...
    async fn fetch_token(&self) -> Result<TokenResult> {
        ...
        let details: StandardDeviceAuthorizationResponse = client
            ...
            .map_err(|e| {
                AppError::DeviceCodeAuthFailed(format!(
    ...
    fn display_instructions(&self, details: &StandardDeviceAuthorizationResponse) {
        ...
        eprintln!(
            "  {} {}",
            style("[*]").cyan(),
            style("Azure Authentication Required").bold()
        );
        eprintln!(
            "{}",
            style("======================================================================").cyan()
...
mod tests {
    ...
    fn test_device_code_auth_creation() {
        let auth = DeviceCodeAuth::new("tenant-id".to_string(), None, &Cloud::Global);
        assert!(auth.is_ok());
    ...
    fn test_china_cloud_scope() {
        let auth = DeviceCodeAuth::new("tenant-id".to_string(), None, &Cloud::China).unwrap();
